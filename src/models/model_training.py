...
formatter = logging.Formatter('%(asctime)s - %(name)s - %(message)s')  # Fixed format string
...
def yaml_params(path: str) -> float:
    try:
        ...
        logger.error(f"Missing key in the YAML file: {e}")  # Corrected error handling
        raise
...
def train(n_estimators: float, train: pd.DataFrame, learning_rate: float, max_depth: float) -> GradientBoostingClassifier:
    try:
        ...
        gbm_model.fit(train.iloc[:, 0:-1], train['output'])  # Assumes target column is 'output'
        ...
        logger.error(f"Missing target column 'output': {e}")  # Added error for missing target column
        raise
    except Exception as e:
